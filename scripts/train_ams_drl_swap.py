...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--def_first_from_cycle2", action="store_true",
                   help="2026-05-11 (사용자 spec). cycle 1은 기존 attacker→defender 순서. "
                        "cycle 2+는 defender→attacker 순서로 swap. 마지막 학습=attacker → "
                        "h2h에서 attacker가 defender_v_K에 over-fit한 상태로 측정 → "
                        "attacker 우세 측정 가능 (기존은 defender 우세).")
...
def main() -> int:   # noqa: PLR0912 — orchestrator drives 3-phase per-cycle loop
    ...
    converged = False
    # cycle 1 cfgs.pkl path 저장 — cycle 2+ swap 모드에서 h2h cmd에 사용.
    cur_a_cfgs: Path | None = None
    ...
    for cycle in range(1, args.max_cycles + 1):
        ...
        # ============================================================
        # 2026-05-11 (사용자 spec — swap variant)
        #   cycle 1   : A block only (attacker train, defender skipped)
        #   cycle 2+  : B block first (defender vs cur_a_actor), then A block
        #               (attacker vs new defender_v_K).
        # 마지막 학습 = attacker → h2h(a_v_K, d_v_K)에서 attacker가 d_v_K에
        # over-fit한 상태로 측정 → attacker 우세 측정 가능.
        #
        # ⚠ ANALYSIS NOTE — Cycle 2는 WARMUP, baseline 후보에서 제외:
        #   - cycle 2 defender는 fresh PPO init (cur_d_resume=None) 으로 시작.
        #   - attacker_v1도 50 iter scratch 학습이라 미숙.
        #   - 양쪽 미숙 → cycle 2 h2h metric은 noise 큼.
        #   - B.adaptive (defender_min_winrate)가 추가 학습으로 보강하지만 한계.
        #   - cycle 3+ 부터 cur_d_resume이 채워져 normal alternating dynamics.
        # → Best ckpt 선정 / Nash 균형 분석은 cycle 3 이후에서만.
        # ============================================================
        ...
        if cycle >= 2:
            # ---- B) defender trains vs frozen cur_a_actor (cycle K-1 attacker) ----
            # FP pool: cur_a_actor는 이미 cycle K-1 끝에서 attacker_pool에 append됨.
            # 추가 append 안 함.
            att_args = _attacker_args_for_defender_train(
                args.use_fp, attacker_pool, cur_a_actor, pold=args.pold,
            )
            cmd_b = [
                py, train_d, *att_args,
            ...
            print(f"[ams-s2 cycle {cycle}] defender — vs frozen cur_a_actor "
                  f"(swap mode)", flush=True)
            rc = _run_child(cmd_b)
            if rc != 0:
            ...
            # B.adaptive — defender capture rate vs frozen cur_a_actor.
            if args.defender_min_winrate > 0:
                ...
                while True:
                    ...
                    cmd_b_eval = _h2h_cmd(
                        py, eval_h2h, cur_a_actor, d_actor_dst,
                        eval_b_json, args, cur_a_cfgs,
                    )
                    rc = _run_child(cmd_b_eval)
                    if rc != 0:
                        _fail(f"B.check eval r{b_round} rc={rc}; cmd: {' '.join(cmd_b_eval)}", cycle=cycle)
                    if not eval_b_json.is_file() or eval_b_json.stat().st_size == 0:
                        _fail(f"B.check h2h r{b_round} missing/empty: {eval_b_json}", cycle=cycle)
                    with open(eval_b_json) as f:
                        last_b_check = json.load(f)
                    last_b_check_path = eval_b_json
                    def_wr = last_b_check["defender_winrate"]
                    print(f"[ams-s2 cycle {cycle}] B.check r{b_round}: "
                          f"def_wr={def_wr:.3f} (target ≥ {args.defender_min_winrate})", flush=True)
                    ...
                    if rc != 0:
                        _fail(f"B.adaptive train_defender rc={rc}; cmd: {' '.join(cmd_extra)}", cycle=cycle)
                    new_d = _find_actor_pt(extra_dir, "defender")
                    new_d_model = _find_model_pt(extra_dir)
                    if new_d is None or new_d_model is None:
                        _fail(f"B.adaptive defender missing artifacts under {extra_dir}", cycle=cycle)
                    d_actor_dst = _copy_to(logs_root / f"defender_v{cycle}.pt", new_d)
                    d_model_dst = _copy_to(logs_root / f"model_defender_v{cycle}.ckpt", new_d_model)

            # alias for A block: cur_d_actor = 방금 학습한 cycle K defender
            cur_d_actor = d_actor_dst

        # ---- A) attacker trains vs (cycle 1: stationary | cycle 2+: new defender) ----
        ...
        # ---- C) Cycle h2h.json ----
        if cycle == 1:
            # cycle 1: defender 미학습 → h2h skip.
            print(f"[ams-s2 cycle 1] swap mode — attacker only "
                  f"(defender skipped). h2h skipped.", flush=True)
        else:
            # cycle 2+: fresh h2h(a_v_K, d_v_K).
            # a_v_K가 d_v_K에 over-fit한 상태 → attacker 우세 측정 가능.
